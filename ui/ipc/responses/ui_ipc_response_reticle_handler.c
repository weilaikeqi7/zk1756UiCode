// ui_ipc_response_reticle_handler.c
// 分划板相关请求响应处理
//
// 当前原则：
 ...
 * 只有整表不存在对应项时，才退回使用 result->distanceValue。
 */
...
ROE_S32 UiIpcHandleResponseSetReticleCommonConfig(ROE_U8 * msgData)
{
    ...
    UiResponseSetReticleCommonConfig * result = (UiResponseSetReticleCommonConfig *)msgData;
...
{
    if(msgData == NULL) return ROE_FAILURE;
    UiResponseWeaponMarkConfigOperate * result = (UiResponseWeaponMarkConfigOperate *)msgData;
    char dist_buf[160];
    format_dist_array_u16(dist_buf, sizeof(dist_buf), result->shootDistanceCount, result->shootDistances);
    LV_LOG_USER(
        "[RETICLE][RSP][3.62 weaponCfg] result=%u video=%u weapon=%u defaultWeapon=%u style=%u color=%u defaultDistanceIndex=%u %s",
        (unsigned)result->result,
        ...
        (unsigned)result->defaultDistanceIndex,
        dist_buf);
    if(result->result != 0) return ROE_FAILURE;
    ...
    gc->default_idx = proto_idx_to_ui(result->defaultDistanceIndex);
...
{
    if(msgData == NULL) return ROE_FAILURE;
    UiResponseWeaponSetReticleStyle * result = (UiResponseWeaponSetReticleStyle *)msgData;
...
{
    if(msgData == NULL) return ROE_FAILURE;
    UiResponseWeaponSetReticleColor * result = (UiResponseWeaponSetReticleColor *)msgData;
...
{
    UI_IPC_RETURN_RESPONSE(msgData, UiResponseWeaponSetReticleBrightness);
}

/* 处理 3.65：设置优先距离。
 * 默认索引以下位机返回的 defaultDistanceIndex 为准。 */
ROE_S32 UiIpcHandleResponseWeaponSetDefaultShootDistance(ROE_U8 * msgData)
{
    if(msgData == NULL) return ROE_FAILURE;
    UiResponseWeaponSetDefaultShootDistance * result = (UiResponseWeaponSetDefaultShootDistance *)msgData;
    LV_LOG_USER(
        "[RETICLE][RSP][3.65 defaultDistance] result=%u video=%u weapon=%u distanceIndex=%u distanceValue=%u abs=(%d,%d) rel=(%d,%d)",
        (unsigned)result->result,
        (unsigned)result->videoChannel,
        (unsigned)result->weaponIndex,
        (unsigned)result->defaultDistanceIndex,
        (unsigned)result->defaultDistance,
    ...
    reticle_distance_entry_t * e = get_dist_entry(gc, result->defaultDistanceIndex);
    apply_zero_to_entry(
        e,
...
{
    if(msgData == NULL) return ROE_FAILURE;
    UiResponseWeaponOperateShootDistance * result = (UiResponseWeaponOperateShootDistance *)msgData;
    char dist_buf[160];
    format_dist_array_u16(dist_buf, sizeof(dist_buf), result->shootDistanceCount, result->shootDistances);
    LV_LOG_USER(
        "[RETICLE][RSP][3.66 distOp] result=%u video=%u weapon=%u opDistIndex=%u opDistValue=%u defaultDistanceIndex=%u abs=(%d,%d) rel=(%d,%d) %s",
        (unsigned)result->result,
        (unsigned)result->videoChannel,
        (unsigned)result->weaponIndex,
        (unsigned)result->distanceIndex,
        (unsigned)result->distanceValue,
        (unsigned)result->defaultDistanceIndex,
        (int)result->absX,
        (int)result->absY,
        (int)result->relX,
    ...
    uint8_t op_tag = proto_idx_to_ui(result->distanceIndex);
    ...
    if(dist_op == 0) {
        ...
        reticle_distance_entry_t * e = get_dist_entry(gc, result->distanceIndex);
        if(e) {
    ...
    /* 默认标签按协议返回的 defaultDistanceIndex 生效，它表示“标签身份”，不是当前排序位置。 */
    gc->default_idx = proto_idx_to_ui(result->defaultDistanceIndex);
...
{
    if(msgData == NULL) return ROE_FAILURE;
    UiResponseWeaponSetShootPosition * result = (UiResponseWeaponSetShootPosition *)msgData;
    LV_LOG_USER(
        "[RETICLE][RSP][3.67 shootPos] result=%u video=%u weapon=%u distanceIndex=%u distanceValue=%u abs=(%d,%d) rel=(%d,%d)",
        (unsigned)result->result,
        (unsigned)result->videoChannel,
        (unsigned)result->weaponIndex,
        (unsigned)result->distanceIndex,
    ...
    reticle_gun_cfg_t * gc = &cfg->guns[gun];
    reticle_distance_entry_t * e = get_dist_entry(gc, result->distanceIndex);
    apply_zero_to_entry(e, result->distanceValue, result->absX, result->absY, result->relX, result->relY);
    reticle_distance_entry_t * de = reticle_distance_mgr_entry_by_tag(proto_idx_to_ui(result->distanceIndex));
    if(de) apply_zero_to_entry(de, result->distanceValue, result->absX, result->absY, result->relX, result->relY);

    reticle_feature_set_active_distance_tag(proto_idx_to_ui(result->distanceIndex));
    reticle_feature_sync_selected_to_active_tag();
    reticle_feature_mark_user_dirty();
    reticle_feature_refresh_calibration_from_model();
    return ROE_SUCCESS;
...
{
    if(msgData == NULL) return ROE_FAILURE;
    UiResponseWeaponSetShootZero * result = (UiResponseWeaponSetShootZero *)msgData;
    LV_LOG_USER(
        "[RETICLE][RSP][3.68 zero] result=%u video=%u weapon=%u distanceIndex=%u distanceValue=%u abs=(%d,%d) rel=(%d,%d)",
        (unsigned)result->result,
        (unsigned)result->videoChannel,
        (unsigned)result->weaponIndex,
        (unsigned)result->distanceIndex,
...
{
    if(msgData == NULL) return ROE_FAILURE;
    UiResponseWeaponSaveConfig * result = (UiResponseWeaponSaveConfig *)msgData;
